...
#endif
    SDL_RestoreWindow(m_window);
    SDL_SetWindowPosition(m_window, x, Core::Max(y, borderH));
...
void CWindow::GetPositionSize(int *x, int *y, int *width, int *height)
{
    ...
    *y = Core::Max(*y, borderH);
    SDL_GetWindowSize(m_window, width, height);
}

void CWindow::SetMinSize(const Core::Vec2<i32> &newMinSize)
{
    if (m_Size.x < newMinSize.x || m_Size.y < newMinSize.y)
    {
        int width = m_Size.x;
        int height = m_Size.y;

        if (width < newMinSize.x)
        {
            width = newMinSize.x;
        }

        if (height < newMinSize.y)
        {
            height = newMinSize.y;
        }
...
{
    if (m_Size.x > newMaxSize.x || m_Size.y > newMaxSize.y)
    {
        int width = m_Size.x;
        int height = m_Size.y;
        ...
        int borderH = 0;
#endif
        SDL_SetWindowMaximumSize(m_window, newMaxSize.x, newMaxSize.y - borderH);
    }
