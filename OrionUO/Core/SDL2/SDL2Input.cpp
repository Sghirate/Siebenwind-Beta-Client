...
void SDL2Input::Init()
{
    ...
    {
        LOG_ERROR(
            "Core::SDL2", "Could not initialize game controller subsystem! (%s)", SDL_GetError());
    }
    ...
    switch (a_event->type)
    {
        ...
        case SDL_MOUSEWHEEL:
        {
            ...
            mouseEvent.wheel.delta = TMouseWheelDelta(
                a_event->wheel.direction == SDL_MOUSEWHEEL_FLIPPED ? (-a_event->wheel.x) :
                                                                     a_event->wheel.x,
                a_event->wheel.direction == SDL_MOUSEWHEEL_FLIPPED ? (-a_event->wheel.y) :
                                                                     a_event->wheel.y);
        ...
        case SDL_KEYUP:
        {
            ...
            keyEvent.scancode =
                SDL2Keyboard::SDLScanCodeToCoreScanCode(a_event->key.keysym.scancode);
        ...
        case SDL_TEXTINPUT:
        {
            ...
            static std::wstring wbuffer;
            DecodeUTF8(std::string(a_event->text.text), wbuffer);
            textEvent.wtext  = wbuffer.data();
        ...
        case SDL_CONTROLLERAXISMOTION:
        {
            ...
            switch (gamepadEvent.axis.axis)
            {
                case EGamepadAxis::LeftX:
                    gamepad.sticks[(size_t)EGamepadStick::Left].x = gamepadEvent.axis.value;
                    break;
                case EGamepadAxis::LeftY:
                    gamepad.sticks[(size_t)EGamepadStick::Left].y = gamepadEvent.axis.value;
                    break;
                case EGamepadAxis::RightX:
                    gamepad.sticks[(size_t)EGamepadStick::Right].x = gamepadEvent.axis.value;
                    break;
                case EGamepadAxis::RightY:
                    gamepad.sticks[(size_t)EGamepadStick::Right].y = gamepadEvent.axis.value;
                    break;
                case EGamepadAxis::LeftTrigger:
                    gamepad.triggers[(size_t)EGamepadStick::Left] = gamepadEvent.axis.value;
                    break;
                case EGamepadAxis::RightTrigger:
                    gamepad.triggers[(size_t)EGamepadStick::Right] = gamepadEvent.axis.value;
                    break;
        ...
        case SDL_CONTROLLERBUTTONUP:
        {
            ...
            gamepadEvent.button.button =
                SDL2Gamepad::SDLButtonToCoreButton(a_event->cbutton.button);
...
void Input::Shutdown()
{
    SDL2Input::Get().Shutdown();
}
IMouse* Input::GetMouse(u8 a_index)
{
    return &SDL2Input::Get().GetMouse();
}
IKeyboard* Input::GetKeyboard(u8 a_index)
{
    return &SDL2Input::Get().GetKeyboard();
}
IGamepad* Input::GetGamepad(u8 a_index)
{
    return a_index < SDL2Input::kMaxGamepads ? &SDL2Input::Get().GetGamepad(a_index) : nullptr;
}
const char* Input::GetKeyName(EKey a_key)
{
    return SDL2Keyboard::GetCoreKeyName(a_key);
}
void Input::RegisterMouseListener(IMouseListener* a_listener)
{
    SDL2Input::Get().RegisterMouseListener(a_listener);
}
void Input::UnregisterMouseListener(IMouseListener* a_listener)
{
    SDL2Input::Get().UnregisterMouseListener(a_listener);
}
void Input::RegisterKeyboardListener(IKeyboardListener* a_listener)
{
    SDL2Input::Get().RegisterKeyboardListener(a_listener);
}
void Input::UnregisterKeyboardListener(IKeyboardListener* a_listener)
{
    SDL2Input::Get().UnregisterKeyboardListener(a_listener);
}
void Input::RegisterGamepadListener(IGamepadListener* a_listener)
{
    SDL2Input::Get().RegisterGamepadListener(a_listener);
}
void Input::UnregisterGamepadListener(IGamepadListener* a_listener)
{
    SDL2Input::Get().UnregisterGamepadListener(a_listener);
}
